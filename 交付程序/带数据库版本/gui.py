...
class userGui(object):
    ...
    def config(self):
        ...
        ####################数据库测试连接是否成功###############################
        self._b_db_connect=self._cls_db.connect_db()
        if not self._b_db_connect:
            self._app.warningBox('警告','数据库连接不成功，无法获取电源板的PCBA版本')
        ...
        if btn == "转换":
            ...
            if all(entries.values()):
                ...
                if df is None:
                    ...
                    # print(df['TraceData'])
                    # g=df['TraceData']
                    # a为recoder_process实例，f为DataFrame
                    a,f=converter_data_to_RecodeData(df)
                    pdSeries_pcba_ver=None

                    if a.all_data!=[]:
                        if self._b_db_connect:
                            pdSeries_pcba_ver=add_db_data_column(f,self._cls_db)
                            write_db_data_to_recoderformat(pdSeries_pcba_ver,a)
                        a.ex_path=self._app.getEntry('ExportFolder')
                        a.template_path=self._app.getEntry('template_user_fileentry')
                        if a.exception[0]:
                            print(a.exception[1])
                            sys.exit(0)
                        a.create_new_file()
